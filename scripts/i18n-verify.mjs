...
function usage() {
  console.log(`Usage:
  node scripts/i18n-verify.mjs [options]

Options:
  --write               Record completed reviews in upstream/i18n.zh.json
  --path=<doc>          A reviewed source path (repeatable)
  --all-reviewed        Explicitly attest that every page has been reviewed
...
function parseArgs(argv) {
  ...
  for (const arg of argv) {
    ...
    } else if (arg === '--all-reviewed') {
      options.allReviewed = true
    } else if (arg === '--write') {
      options.write = true
    } else if (arg === '--dry-run') {
      options.dryRun = true
  ...
  if (options.write && !options.allReviewed && options.paths.length === 0) {
    throw new Error('Use --path=<reviewed-page> (repeatable) or --all-reviewed after reviewing every page.')
  }
  return options
}
...
function isTrackedDocPath(docPath) {
  return (
    docPath === 'community-resources.md' ||
    (docPath.startsWith('framework/react/') && !docPath.startsWith('framework/react/examples/')) ||
...
async function writeMetadata(result, options) {
  ...
  const nextTranslations = Object.fromEntries(Object.entries(result.translations)
    .filter(([key]) => result.zhFiles.includes(key) && result.currentHashes[key]))
  for (const selected of options.paths) {
    if (!result.currentHashes[selected]) throw new Error(`Unknown or missing translation: ${selected}`)
  }
  let updated = 0
  let unchanged = 0

  for (const relPath of result.zhFiles) {
    if (!options.allReviewed && !options.paths.includes(relPath)) continue
    if (!result.currentHashes[relPath]) continue
    const translation = await readFile(path.join(resolveFromRoot(options.root, options.docsRoot), options.locale, relPath), 'utf8')
    if (/translation-status:\s*draft/.test(translation)) throw new Error(`Translate the draft before recording a review: ${relPath}`)
