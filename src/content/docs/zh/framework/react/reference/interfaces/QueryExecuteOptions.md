...
定义于： [packages/query-core/src/types.ts:629](https://github.com/TanStack/query/blob/main/packages/query-core/src/types.ts#L629)
...
| <a id="select"></a> `select?` | (`data`: `TQueryData`) => `TData` | `undefined` | 可转换或选择查询函数返回的数据的一部分。它影响本次调用解析出的值，不影响查询缓存中存储的数据。 |
| <a id="staletime"></a> `staleTime?` | \| `number` \| `"static"` \| (`query`: [`Query`](../classes/Query.md)\<`TQueryFnData`, `TError`, `TQueryData`, `TQueryKey`\>) => `number` \| `"static"` | `undefined` | 数据被视为 stale 前的毫秒数；数据仍为 fresh 时，会从缓存中返回。 |
