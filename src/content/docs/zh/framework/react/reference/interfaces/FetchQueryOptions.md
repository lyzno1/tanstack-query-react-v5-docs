...
定义于： [packages/query-core/src/types.ts:654](https://github.com/TanStack/query/blob/main/packages/query-core/src/types.ts#L654)
...
| <a id="staletime"></a> ~~`staleTime?`~~ | \| `number` \| `"static"` \| (`query`: [`Query`](../classes/Query.md)\<`TQueryFnData`, `TError`, `TData`, `TQueryKey`\>) => `number` \| `"static"` | `undefined` | 数据被视为 stale 前的毫秒数；数据仍为 fresh 时，会从缓存中返回。 |
