...
定义于： [packages/react-query/src/types.ts:413](https://github.com/TanStack/query/blob/main/packages/react-query/src/types.ts#L413)

`useMutation` 接受的选项。它与 `@tanstack/query-core` 中的 [MutationObserverOptions](MutationObserverOptions.md) 相同，
...
| <a id="throwonerror"></a> `throwOnError?` | `boolean` \| (`error`: `TError`) => `boolean` | `false` | 决定是否抛出错误而非设置 `error` 属性。设为 `true` 时，所有错误都会抛给最近的 Error Boundary；设为函数时，接收错误并返回布尔值，决定抛出（`true`）还是作为状态返回（`false`）。 |
