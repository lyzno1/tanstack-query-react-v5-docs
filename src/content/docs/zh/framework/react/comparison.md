...
> **<sup>4</sup> 使用前查询配置** - 这是一个“花哨”的说法，指的是在查询和 mutation 真正使用前就预先配置其行为。例如，可先设置完整默认配置，使用时只需 `useQuery({ queryKey })`，而不必每次都传 fetcher 和/或 options。SWR 仅提供该特性的部分能力：可全局预设默认 fetcher，但不能按查询粒度配置，更无法用于 mutation。

> **<sup>5</sup> mutation 后自动重新获取** - 若要在 mutation 发生后实现真正自动重新获取，通常需要 schema（例如 GraphQL 的 schema）以及一套启发式规则，用于帮助库识别 schema 中的实体与实体类型。
